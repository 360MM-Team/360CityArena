...
if [[ -z "${MODEL_NAME}" ]]; then
  echo "[internvl3.5] ERROR: MODEL_NAME (or LLM_MODEL) is required." >&2
  exit 1
fi

if [[ -z "${EFFECTIVE_API_KEY}" ]]; then
  echo "[internvl3.5] ERROR: LLM_API_KEY is not set for model (${MODEL_NAME})." >&2
  echo "Export LLM_API_KEY before running (例: export LLM_API_KEY=EMPTY)." >&2
  exit 1
fi

if [[ -z "${LLM_API_BASE}" ]]; then
  echo "[internvl3.5] ERROR: LLM_API_BASE must be set for model (${MODEL_NAME})." >&2
  echo "Export LLM_API_BASE (例: http://127.0.0.1:8000/v1) before running." >&2
  exit 1
fi

if [[ -z "${LLM_PRETRAINED}" ]]; then
  echo "[internvl3.5] ERROR: LLM_PRETRAINED must be set for model (${MODEL_NAME})." >&2
  echo "Export LLM_PRETRAINED (例: OpenGVLab/InternVL3.5-8B) before running." >&2
  exit 1
fi
